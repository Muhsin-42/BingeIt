...
function PostCard({post,getAllPosts}) {
  ...
  const [liked,setLiked] = useState(post?.likes?.hasOwnProperty(currentUser._id));
  const [likesCount,setLikesCount] = useState(post?.likes ? Object.keys(post?.likes).length : 0);
  ...
  const handleLike = async () =>{
    setLiked(prev=> !prev);
    const response = await axios.patch(`/api/post/like/${post._id}`,{userId : currentUser._id},{
  ...
  const handlePostComment = async () =>{
    ...
    try{
      const response = await axios.patch(`/api/post/comment/${post._id}`, {comment: comment, currentUserId : currentUser._id} ,
      {
        headers :{ "Content-Type" : "application/json", Authorization:  `Bearer ${token}`}
      });
      setComment('');
    ...
    <div className='postCard'>
       ...
       <div className="level1">
          <div className="set1 d-flex">
            ...
            <div className="nameUsername mx-2 d-flex " 
        ...
        <div className="level4 mt-2">
          {
            liked ? <FavoriteIcon color='error' onClick={handleLike} className='cursor-pointer'/>
                  : <FavoriteBorderIcon onClick={handleLike} className='cursor-pointer'/>
          }
          <span className='mx-2'>{likesCount} likes</span>
          <InsertCommentIcon onClick={()=>setShowComments(!showComments)} className='cursor-pointer mx-2 '/>

          {
            currentUser._id !== post?.author?._id ?
              ...
              <Box sx={style}>
                ...
                <ListItemButton onClick={() => handleReport('Irrelevant Topic')} component="a" href="#simple-list">
                  <ListItemText primary="Irrelevant Topic" />
                </ListItemButton>
                <ListItemButton onClick={() => handleReport('Scam or fraud')} component="a" href="#simple-list">
                  <ListItemText primary="Scam or fraud" />
                </ListItemButton>
                <ListItemButton onClick={() => handleReport('Hate speech or symbol')} component="a" href="#simple-list">
                  <ListItemText primary="Hate speech or symbol" />
                </ListItemButton>
                <ListItemButton onClick={() => handleReport('False Information')} component="a" href="#simple-list">
            ...
            :
              <>
                  <DeleteIcon onClick={deletePost} className='cursor-pointer' ></DeleteIcon>
              </>
          }
...

          {
            ...
            <div className="level5">
              ...
              <div className="set2">
                <input type="text" value={comment} onChange={(e)=>setComment(prev=> e.target.value)} className='input-group'  placeholder='Write a comment' />
              </div>
              <div className="set3">
                <button onClick={handlePostComment} className='btn btn-success'>POST</button>
              </div>
            </div>
          }
          {
