...
// import {originals,ActionMovies,Comedy,Romance,Horror,Documentary} from '../../movieApi/urls'
let friendDetails;
...
function Profile() {
  ...
  const handleFollow = async (friendId) => {
      try {
          setLoading(true);
          const response = await axios.patch(`api/user/follow/${id}`,{ currentUser:  currentUserRedux._id}, {
              headers: {
          ...
          const postNotification = await axios.post(`api/notification/follow/${id}`,{notificationData},{
            headers: {
                "Content-Type": "application/json",
                'Authorization': `Bearer ${token}`,
            },
  ...
  const handleUnfollow = async (friendId) => {
      try {
          setLoading(true)
          const response = await axios.patch(`api/user/unfollow/${id}`,{ currentUser:  currentUserRedux._id}, {
              headers: {
                  "Content-Type": "application/json",
                  'Authorization': `Bearer ${token}`,
              },
  ...
  useEffect(()=>{
    if(friend._id == currentUserRedux._id){
  ...
  return (
    ...
    <div id='profile'>
        <ProfilePics bg={bg} dp={dp} currentUserRedux={currentUserRedux} profileUser={friend} />
         ...
         {/* Follow / UnFollow / Edit */}
          <div className="right">
                {currentUserRedux._id !== friend._id && (
                    <>
                    { <button className='btn btn-outline-primary' onClick={toggleFollow}> {following ? 'Unfollow' : 'follow' } </button>} 
                    </>
                )}
                {currentUserRedux._id === friend._id && (
                    <>
                     <EditProfile open={editProfile} setOpen={setEditProfile} />
                        <button className='btn btn-outline-primary' onClick={e => setEditProfile(true)}>Edit</button>
                    </>
                )}
          </div>
        </div>
        ...
        <div className="infoBar  mt-4 row ">
            <div onClick={()=>toggleInfoBar('movie')} className="card-1 col col-12 col-lg-3 col-md-12 col-sm-12" 
                    style={{
                        background: activeTab === 'movie' ? 'white' : '#272727',
                        color: activeTab === 'movie' ? 'black' : 'white',
                        pointerEvents: activeTab === 'movie' ? 'none' : 'auto'
                      }}>
                    Movies <br />{friend?.watched?.length}
            </div>

            <div onClick={()=>toggleInfoBar('followers')}  className="card-2 col col-6 col-lg-2 col-md-6 col-sm-6"
                     style={{
...

        {
            ...
            : activeTab == 'followers' ?
                <Followers profileUser={friend} currentUser={currentUserRedux} activeTab={activeTab} />
                :   
                <Followers profileUser={friend} currentUser={currentUserRedux} activeTab={activeTab} />
        }
    </div>
