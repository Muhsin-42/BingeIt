...
function SingleMovie() {
  ...
  const getMovieDetails = async () => {
    try {
      const response = await Movieaxios.get(
        `${movieBaseUrl}/movie/${movieId}?api_key=${API_KEY}&language=en-US`
      );
  ...
  const handleUnFavouriteClick = async () => {
    try {
      ...
      const response = await axios.delete(
        `api/user/favourite/${movieId}/${currentUser._id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleUnWishlistClick = async () => {
    try {
      const response = await axios.delete(
        `api/user/wishlist/${movieId}/${currentUser._id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleUnWatchedClick = async () => {
    try {
      // setHeartClicked(!heartClicked);
      const response = await axios.delete(
        `api/user/watched/${movieId}/${currentUser._id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        <div className="mainContents">
          ...
          <div className="right  ">
            ...
            <div className="row">
              ...
              <p className="movieOverview text-white p-3">
              ...
              <div className="row">
                {currentUser?.favourite.includes(movieDetails?.id + "") ? (
                  <button
                    ...
                    className="col mx-3 btn btn-danger"
                  >
                    <i className="bi bi-heart-fill"></i> Favourite
                  </button>
                ) : (
                  <button
                    onClick={handleFavouriteClick}
                    className="col mx-3 btn btn-outline-danger"
                  >
                ...
                {currentUser?.wishlist.includes(movieDetails?.id + "") ? (
                  ...
                  <button
                    ...
                    className="col mx-3 btn btn-outline-danger"
                  >
                    <i className="bi bi-bookmark"></i> Wishlist
                  </button>
                )}
                {currentUser?.watched.includes(movieDetails?.id + "") ? (
                  <button
                    onClick={handleUnWatchedClick}
                    className="col mx-3 btn btn-success"
                  >
                    <i className="bi bi-check-circle-fill"></i> Watched
                  </button>
                ) : (
                  <button
                    onClick={handleWatchedClick}
                    className="col mx-3 btn btn-outline-success"
                  >
                    <i className="bi bi-check-circle"></i> Watched
                  </button>
                )}
              </div>
              <button
                onClick={handleTrailerClick}
                className="btn btn-outline-danger mx-auto m-3 w-75"
              >
