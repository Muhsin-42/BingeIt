  ...
  const handlePostReply = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `api/user/reply/${review._id}/${currentUser._id}`,
        { data: replyMsgInput },
        {
          headers: {
            "Content-Type": "application/json",
      ...
      if (currentUser?._id?.toString() !== review?.userId?._id?.toString()) {
        ...
        await axios.post(
          `api/notification/follow/${review.userId._id}`,
          { notificationData },
          {
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div className="replyMain bg-dark pb-3">
      ...
      <form onSubmit={handlePostReply} className="add-reply-main row mb-4">
        ...
        <input
          className="col col-6 col-lg-7"
          value={replyMsgInput}
          onChange={(e) => setReplyMsgInput(e.target.value)}
          type="text"
        ...
        <button
          type="submit"
          className="col col-2 col-lg-2 btn btn-outline-danger"
        >
          reply
        </button>
      </form>
